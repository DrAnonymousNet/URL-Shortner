...
const Input: React.FC<Props> = ({
  ...
  const handleEyesClick = (e: React.FormEvent) => {
    ...
    if (inputType === undefined && previousSibling.type == "password")
      inputType = previousSibling.type;
    if (inputType) {
    ...
    <label htmlFor={labelFor} className="text-left text-xs inline-block w-full">
      ...
      <p className="text-left relative h-full">
        <input
          ...
          className={`outline-none py-2 px-3 border-solid border-[1px]  rounded-md  block text-base w-full focus:outline-none focus:border-[#0b1a30] ${
            showRedBorder ? "border-red-400" : "border-secondary-600"
          } ${className}`}
